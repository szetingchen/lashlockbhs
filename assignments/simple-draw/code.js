    ...
    for(let x=0; x<=border+offsetx; x++){
      if(x<=500+offsetx&&y<=600+offsety&&x-offsetx>=0&&y-offsety>=0){
        ...
        if(pixelinset===0){
          ...
          drawLine(x-offsetx, y-offsety, x+1-offsetx, y-offsety, rgb(pixelinset*1+(iterations/iterations*1.001), pixelinset*1+(iterations/iterations*1.001),pixelinset*1+(iterations/iterations*1.001)))
          count++
        }
      }
    }
...
const x = -63
const y = -20
drawmandel(100000, 60000, x+width/2, y+height/2)
