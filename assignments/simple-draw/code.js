...
const drawmandel = (iterations, border, zoomx, zoomy) =>{
  const color = 'black'
  const offsetx = ((zoomx/border)*border)-Math.abs((border*(border-600)/600))
  const offsety = ((zoomy/border)*border)-Math.abs((border*(border-600)/600))
  let xmath = 0
  let ymath = 0
    ...
    for(let x=0; x<=border+offsetx; x++){
      if((x<=530+offsetx||y<=600+offsety)&&x-offsetx>=0&&y-offsety>=0){
        ...
        if(pixelinset===0){
          ...
          drawLine(x-offsetx, y-offsety, x+1-offsetx, y-offsety, rgb(6*pixelinset, 14*pixelinset, 12*pixelinset))
        }
      }
    }
  }
