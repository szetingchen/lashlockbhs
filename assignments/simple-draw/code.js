...
const drawmandel = (iterations, borderx, bordery, centerx, centery, zoom) =>{
  ...
  for(let y = 0; y<=bordery; y++){
    for(let x= 0; x<=borderx; x++){
      ...
      if(pixelinset===0){
        drawLine(x-(borderx/2), y-(bordery/2), x+1-(borderx/2), y-(bordery/2), color)
        count++;
      }
      else if(pixelinset>0){
        drawLine(x-(borderx/2), y-(bordery/2), x+1-(borderx/2), y-(bordery/2), 'hsl(' + 2+(pixelinset*2) + ', 100%, 50%)')
        count++;
      }
    }
  }
...
const x = 0
const y = 200
drawmandel(1000, 600, 600, x+width/2, y+height/2, 3)
