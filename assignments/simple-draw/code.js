...
const drawmandel = (iterations, border, zoomx, zoomy) =>{
  ...
  for(let y =0; y<=border+offsety; y++){
    for(let x=0; x<=border+offsetx; x++){
      if((x<=border+offsetx||y<=border+offsety)&&x-offsetx>=0&&y-offsety>=0){
        xmath=-2+(4/border)*x
        ymath=2-(4/border)*y
        let pixelinset = isPixelInSet([0,0], [xmath, ymath], iterations)
        
        if(pixelinset===0){
          drawLine(x-offsetx, y-offsety-200, x+1-offsetx, y-offsety-200, color)
        }
        else if(pixelinset>0){
          drawLine(x-offsetx, y-offsety-200, x+1-offsetx, y-offsety-200, rgb(2*pixelinset, 45*pixelinset, 76*pixelinset))
        }
      }
    }
  }
...
const x = 100
const y = 100
drawmandel(40, 1200, x+width/2, y+height/2)
