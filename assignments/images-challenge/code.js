...
const concentricCircles = (num, color1, color2) =>{
  ...
  for (let i = 0; num>i; i++){
    if(i%2===0){
      drawFilledCircle(width/2, height/2, height/2-inc*i, color1)
    }
    else{
      drawFilledCircle(width/2, height/2, height/2-inc*i, color2)
    }
  }
}
...
const fillWithCircles = (r, color) =>{
  ...
  for (let y = 0; numy>y; y+=d){
    for (let x = 0; numx>x; x+=d){
